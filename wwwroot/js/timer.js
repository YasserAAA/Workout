...
export const Timer = {
    ...
    startRepTimer(state) {
        ...
        const updateRepTimer = (currentTime) => {
            if (!state.isPaused) {
                ...
                DOM.currentTime.textContent = `Elapsed: ${Utils.formatTime(seconds)}`;

                // Track active workout time for rep mode
                if (!state.lastUpdateTime) state.lastUpdateTime = currentTime;
                state.activeWorkoutTime += (currentTime - state.lastUpdateTime);
    ...
    togglePause(state) {
        if (state.isPaused) {
            // Resume - check if we're in background first
            if (document.hidden) {
