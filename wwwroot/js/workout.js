...
export const Workout = {
    ...
    handleRepExercise(state, item) {
        ...
        DOM.currentTitle.textContent = item.name;
        DOM.currentTitle.style.color = item.color;
        DOM.currentMode.textContent = `${item.reps} reps`;
    ...
    updateNextUpText(state) {
        ...
        if (nextItems.length > 0) {
            nextItems.forEach((item, index) => {
                ...
                if (item.type === 'exercise') {
                    paragraph.textContent = `${index + 1}. ${item.name} - ${item.mode === 'reps' ? `${item.reps} reps` : `${item.time} seconds`}`;
                } else if (item.type === 'break') {
                    paragraph.textContent = `${index + 1}. Break - ${item.time} seconds`;
                } else if (item.type === 'group') {
                    paragraph.textContent = `${index + 1}. ${item.name} - ${item.reps} rounds`;
                }

                DOM.nextUpContent.appendChild(paragraph);
            });
        } else {
            const paragraph = document.createElement('p');
            paragraph.textContent = 'End of Routine';
            DOM.nextUpContent.appendChild(paragraph);
        }
    }
