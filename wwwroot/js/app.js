...
class GymTimerApp {
                ...
                if (registration && registration.active) {
                    ...
                    channel.port1.onmessage = (event) => {
                        if (event.data && event.data.version) {
                            DOM.versionDisplay.textContent = event.data.version;
                        } else {
                            DOM.versionDisplay.textContent = 'v?'; // Fallback
                        }
                    };
                    ...
                    setTimeout(() => {
                        if (DOM.versionDisplay.textContent === 'v') {
                            DOM.versionDisplay.textContent = 'v?';
                        }
                    }, 1000);
                } else {
                    // Service worker not active, use fallback
                    DOM.versionDisplay.textContent = 'v?';
                }
            }).catch(() => {
                // Error getting registration, use fallback
                DOM.versionDisplay.textContent = 'v?';
            });
        } else {
            // Service workers not supported, use fallback
            DOM.versionDisplay.textContent = 'v?';
        }
    }
    ...
    setupEventListeners() {
        ...
        DOM.checkUpdatesBtn.addEventListener('click', () => this.checkForUpdates());
    }
    ...
    checkForUpdates() {
        if ('serviceWorker' in navigator) {
            navigator.serviceWorker.getRegistration().then(reg => {
                if (reg) {
                    reg.update();
                    Utils.showAlert('Checking for updates...', 'info');
                }
            });
        }
    }
